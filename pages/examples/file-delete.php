...
if(isset($_POST['flag']) && $_POST['flag'] === 'file_deletion')
{
    /**
    ...
    if($stmt = $conn -> query($query))
    {
        if($stmt -> rowCount() >= 1) // proceed only of row is found
        {
            ...
            $updated_files = trim(str_replace($file, "", $row['Files']), ", "); // trimming the extra space
            $updated_sizes = trim(str_replace($fileSize, "", $row['Files']), ", "); // updating file sizes also
